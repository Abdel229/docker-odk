...
=======
        ...
        elseif ($this->request->type == 'cinetpay') {

            $this->validate($this->request, [
                "phone_cinetpay" => "required|confirmed|numeric",
                "phone_indicative"=> "required|numeric",
            ]);
          
            //appel cinetpay pour ajouter l'utilisateur 
            $user = User::find(auth()->id());
            

            
            $data=array('prefix' => $this->request->phone_indicative, 'phone' => $this->request->phone_cinetpay, 'name'=> $user->name, 'surname'=> $user->username,'email'=> $user->email,);
            ...
            $result = $cinetPay->AddContact(json_encode($data));
            if($result == 0){
            $user->cinetpay_number = $this->request->phone_cinetpay;
            $user->cinetpay_number_indicative  = $this->request->phone_indicative;
            $user->payment_gateway = 'CinetPay';
            $user->save();
            Session::flash('status', trans('admin.success_update'));
            return redirect()->route("payout.method");
            }else{

                return redirect()->back()
                ->withErrors([
                    'errors' => trans('general.set_payment'),
                ]);
...
=======
    ...
    public function makeWithdrawals()
    {
        if (auth()->user()->payment_gateway != ''
            ...
            if (auth()->user()->payment_gateway == 'PayPal' ) {
                $_account = auth()->user()->paypal_account;
            }else if(auth()->user()->payment_gateway == 'CinetPay' && auth()->user()->cinetpay_number!== 0) {
                
                $_account = auth()->user()->cinetpay_number;
                
              
            }
            else if (auth()->user()->payment_gateway == 'Bank' ){
                $_account = auth()->user()->bank;
            }else{
                return redirect('/settings/payout/method')
                ->withErrors([
