...
class PayPerViewController extends Controller
{
    ...
    public function send(Request $request)
    {
        ...
        if ($request->isMessage) {
            ...
            // Find Post
            $media = Updates::whereId($request->id)
                ->wherePrice($this->request->amount)
                ->where('user_id', '<>', auth()->user()->id)
                ->firstOrFail();

            // Verify that the user has not purchased the content
            if (PayPerViews::whereUserId(auth()->user()->id)->whereUpdatesId($request->id)->first()) {
                return response()->json([
                    "success" => false,
                    "errors" => ['error' => __('general.already_purchased_content')]
                ]);
    ...
    private function sendCinetPay()
    {
        try {
            ...
            $data["amount"] = $this->request->amount*100;
            $data["id_update"] = $this->request->id;
