...
=======
@section('title') {{ $response->title }} | {{ \Stichoza\GoogleTranslate\GoogleTranslate::trans(trans('general.blog'), config("app.locale")) }} @endsection
@section('description_custom'){{strip_tags($response->content)}}@endsection
@section('keywords_custom'){{$response->tags ? $response->tags.',' : null}}@endsection
    ...
    <section class="section section-sm">
        <div class="container">
            <div class="row">
                <div class="@if ($blogs->count() == 0) col-md-12 @else col-md-8 @endif py-5">
                    <div class="row no-gutters border rounded flex-md-row mb-4 shadow-sm h-md-250 position-relative">
                        <div class="card-cover w-100 img-post"
                             style="background: @if ($response->image != '') url({{ Helper::getFile(config('path.admin').$response->image) }})  @endif #505050 no-repeat center center; background-size: cover; "></div>
                        <div class="col p-4 d-flex flex-column position-static">
                            <small
                                class="d-inline-block mb-2">{{ \Stichoza\GoogleTranslate\GoogleTranslate::trans(trans('general.by'), config("app.locale")) }} {{ $response->user()->name }} </small>
                            <h3 class="mb-0">{{ $response->title }}</h3>
                            <div class="mb-3 text-muted">{{ Helper::formatDate($response->date) }}</div>
                            <div class="card-text mb-auto content-p">{!! $response->content !!}</div>
                            ...
                            <div class="mt-4 justify-content-middle">
                                ...
                                <a target="_blank"
                                   ...
                                   class="mr-2">
                                    <i class="fab fa-facebook mr-1"></i> {{\Stichoza\GoogleTranslate\GoogleTranslate::trans(trans('general.share'), config("app.locale"))}}
                                </a>

                                <a target="_blank"
                                   href="https://twitter.com/intent/tweet?url={{url()->current()}}&text={{ $response->title }}">
                                    <i class="fab fa-twitter mr-1"></i> Tweet
                                </a>
                            </div>
                ...
                @if ($blogs->count() != 0)
                    ...
                    <div class="col-md-4 mb-4 py-lg-5">
                        ...
                        <h6 class="mb-3 text-muted font-weight-light">{{ \Stichoza\GoogleTranslate\GoogleTranslate::trans(trans('general.others_posts'), config("app.locale")) }}</h6>

                        @foreach ($blogs as $response)

                            <a href="{{url('blog/post', $response->id).'/'.$response->slug}}">
                                <div class="w-100 d-block"
                                     style="background: @if ($response->image != '') url({{ Helper::getFile(config('path.admin').$response->image) }})  @endif #505050 center center; border-radius: 6px; background-size: cover;">
                                        ...
                                        <div class="media">
                                            <div class="media-body">
                                                <h5 class="media-heading mb-1">
                                                    ...
                                                    <small
                                                        class="text-white w-100 d-block mb-2">{{'@'.$response->user()->name}}
                                                        - {{ Helper::formatDate($response->date) }}</small>
                                                    <p class="text-white font-weight-light">{{ Str::limit(strip_tags($response->content), 60, '...') }}</p>
                                                </h5>
                                            </div>
                                        </div>
