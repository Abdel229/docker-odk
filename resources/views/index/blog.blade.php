...
=======
@section('title') {{ \Stichoza\GoogleTranslate\GoogleTranslate::trans(trans('general.blog'), config("app.locale")) }} -@endsection
...
@section('content')
    <section class="section section-sm">
        <div class="container">
            <div class="row justify-content-center text-center mb-sm">
                <div class="col-lg-12 py-5">
                    <h2 class="mb-0 text-break">{{ \Stichoza\GoogleTranslate\GoogleTranslate::trans(trans('general.latest_blog'), config("app.locale")) }}</h2>
                    <p class="lead text-muted mt-0">{{\Stichoza\GoogleTranslate\GoogleTranslate::trans(trans('general.subtitle_blog'), config("app.locale"))}}</p>
                </div>
            </div>
                    ...
                    @foreach ($blogs as $response)
                        <div class="col-md-6">
                            <div
                                ...
                                <div class="card-cover w-100"
                                     style="height:250px; background: @if ($response->image != '') url({{ Helper::getFile(config('path.admin').$response->image) }})  @endif #505050 center center;"></div>
                                <div class="col p-4 d-flex flex-column position-static">
                                    <small
                                    ...
                                    <a href="{{ url('blog/post', $response->id).'/'.$response->slug }}"
                                       class="stretched-link">{{ \Stichoza\GoogleTranslate\GoogleTranslate::trans(trans('general.continue_reading'), config("app.locale")) }}</a>
                                </div>
                            </div>
                        </div>
                    @endforeach
