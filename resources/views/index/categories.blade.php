...
=======
@section('description_custom'){{\Stichoza\GoogleTranslate\GoogleTranslate::trans($description ? $description : trans('seo.description'), config("app.locale"))}}@endsection
@section('keywords_custom'){{$keywords ? $keywords.',' : null}}@endsection
...
@section('content')
    <section class="section section-sm">
        <div class="container">
            <div class="row justify-content-center text-center mb-sm">
                <div class="col-lg-12 py-5">
                    <h2 class="mb-0 font-montserrat">
                        <img src="{{url('img-category', $image)}}" class="mr-2 rounded" width="30"
                             alt=""/> {{\Stichoza\GoogleTranslate\GoogleTranslate::trans($title, config("app.locale"))}}
                    </h2>
                    <p class="lead text-muted mt-0">({{$users->total()}}
                        ) {{\Stichoza\GoogleTranslate\GoogleTranslate::trans(trans_choice('users.creators_in_this_category',$users->total() ), config("app.locale"))}}</p>
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-md-3 mb-4">
                    ...
                    <button type="button" class="btn-menu-expand btn btn-primary btn-block mb-4 d-lg-none" type="button"
                            data-toggle="collapse" data-target="#navbarFilters" aria-controls="navbarCollapse"
                            aria-expanded="false">
  ...
  <a class="text-muted btn btn-sm bg-white border mb-2 e-none btn-category @if(request()->is('category/'.$slug.''))active-category @endif"
  ...
  <a class="text-muted btn btn-sm bg-white border mb-2 e-none btn-category @if(request()->is('category/'.$slug.'/featured'))active-category @endif"
     href="{{url('category/'.$slug.'','featured')}}">
  ...
  <a class="text-muted btn btn-sm bg-white border mb-2 e-none btn-category @if(request()->is('category/'.$slug.'/more-active'))active-category @endif"
     href="{{url('category/'.$slug.'','more-active')}}">
  ...
  <a class="text-muted btn btn-sm bg-white border mb-2 e-none btn-category @if(request()->is('category/'.$slug.'/new'))active-category @endif"
     href="{{url('category/'.$slug.'','new')}}">
  ...
  <a class="text-muted btn btn-sm bg-white border mb-2 e-none btn-category @if(request()->is('category/'.$slug.'/free'))active-category @endif"
     href="{{url('category/'.$slug.'','free')}}">
        ...
        @endif
              ...
              <span class="btn-block mb-3">
                <i class="fa fa-user-slash ico-no-result"></i>
                            ...
                            <h4 class="font-weight-light">{{\Stichoza\GoogleTranslate\GoogleTranslate::trans(trans('general.not_found_creators_category'), config("app.locale"))}}</h4>
                        </div>
                    </div>
                @endif
            </div>
