    ...
    @foreach ($mediaImageVideo as $media)
        ...
        @if ($media->image != '')
            ...
            <a href="{{ $urlImg }}" class="glightbox w-100" data-gallery="gallery{{$response->id}}">
                <img src="{{$urlImg}}?w=130&h=100"
                     {!! $media->width ? 'width="'. $media->width .'"' : null !!} {!! $media->height ? 'height="'. $media->height .'"' : null !!} data-src="{{$urlImg}}?w=960&h=980"
                     class="img-fluid lazyload d-inline-block w-100 post-image" alt="{{ e($response->description) }}">
            </a>
        @endif
