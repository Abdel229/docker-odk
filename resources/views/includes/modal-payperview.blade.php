                ...
                <div class="card bg-white shadow border-0">
                    ...
                    <div class="card-body px-lg-5 py-lg-5 position-relative">
                        ...
                        <div class="mb-3">
                            ...
                            <strong>{{\Stichoza\GoogleTranslate\GoogleTranslate::trans(trans('general.unlock_content'), config("app.locale"))}}</strong>
                        </div>

                        <form method="post" action="{{ route("send.ppv") }}" id="formSendPPV">
                            ...
                            @foreach (PaymentGateways::where('enabled', '1')->whereSubscription('yes')->get() as $payment)
                                ...
                                @php
                                    ...
                                    if ($payment->type == 'card' ) {
                                        $paymentName = '<i class="far fa-credit-card mr-1"></i> '.\Stichoza\GoogleTranslate\GoogleTranslate::trans(trans('general.debit_credit_card'), config("app.locale")) .' <small class="w-100 d-block">'.__('general.powered_by').' '.$payment->name.'</small>';
                                ...
                                <div class="custom-control custom-radio mb-3">
                                    <input name="payment_gateway_ppv" value="{{$payment->id}}"
                                           id="ppv_radio{{$payment->id}}"
                            ...
                            @if ($settings->disable_wallet == 'on' && Helper::userWallet('balance') != 0 || $settings->disable_wallet == 'off')
                                <div class="custom-control custom-radio mb-3">
                                    <input name="payment_gateway_ppv" @if (Helper::userWallet('balance') == 0) disabled
                                           @endif value="wallet" id="ppv_radio0" class="custom-control-input"
                                           type="radio">
                            ...
                            <div class="text-center">
                                <button type="submit" id="ppvBtn" class="btn btn-primary mt-4 ppvBtn">
                                    <i></i> {{\Stichoza\GoogleTranslate\GoogleTranslate::trans(trans('general.pay'), config("app.locale"))}}
                                    <span class="pricePPV"></span>
                                    <small>{{$settings->currency_code}}</small></button>
