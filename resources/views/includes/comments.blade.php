@if ($response->comments()->count() > $settings->number_comments_show && $counter >= 1)
    <div class="btn-block mb-4 text-center wrap-container" data-total="{{ $response->comments()->count() }}"
        ...
        <a href="javascript:void(0)" class="loadMoreComments">
            — {{ \Stichoza\GoogleTranslate\GoogleTranslate::trans(trans('general.load_comments'), config("app.locale")) }}
...
@foreach ($dataComments as $comment)
    <div class="comments media li-group pt-3 pb-3" data="{{$comment->id}}">
        ...
        <div class="media-body">
            ...
            <span class="small sm-font sm-date text-muted timeAgo"
                  data="{{date('c', strtotime($comment->date))}}"></span>
            @if ($comment->user_id == auth()->user()->id || $response->user()->id == auth()->user()->id)
                <span class="c-pointer small sm-font delete-comment font-weight-bold" data="{{$comment->id}}"><i
        ...
        <i class="@if (auth()->check() && $comment->likes()->whereUserId(auth()->user()->id)->first()) fas fa-heart text-red mr-1 @else far fa-heart mr-1 @endif"></i>
          <span
              class="countCommentsLikes">{{ $comment->likes()->count() != 0 ? $comment->likes()->count() : null }}</span>
      </span>
        </div><!-- media-body -->
    </div>
@endforeach
