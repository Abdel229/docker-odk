									    ...
									    <div class="row">
									      ...
									      <div class="col-auto percentageAppliedTax">
									        <small class="font-weight-bold">
									        	{{ Helper::calculatePercentage($live->price, $tax->percentage) }}
									        </small>
									      </div>
									    </div>
									  </li>
                ...
                <div class="card bg-white shadow border-0">
                    ...
                    <div class="card-body px-lg-5 py-lg-5 position-relative">
                        ...
                        <div class="mb-4 text-center position-relative">
                            <div class="text-center position-relative mb-3 modal-offset">
                                <div class="wrapper-live">
                                    ...
                                    <div class="live-pulse"></div>
                                    <img src="{{Helper::getFile(config('path.avatar').$creator->avatar)}}" width="100"
                                         class="rounded-circle mb-1" alt="">
                                </div>
                            </div>

                            <i class="bi bi-broadcast mr-1"></i>
                            <strong>{{\Stichoza\GoogleTranslate\GoogleTranslate::trans(trans('general.Join_live_stream'), config("app.locale"))}} {{ '@'.$creator->username }}</strong>
                        ...
                        <form method="post" action="{{url('send/payment/live')}}" id="formPayLive">
                            ...
                            <div class="custom-control custom-radio mb-3">
                                <input name="payment_gateway_live" @if (Helper::userWallet('balance') == 0) disabled
                                       @else checked @endif value="wallet" id="live_radio0" class="custom-control-input"
                                       type="radio">
                                <label class="custom-control-label" for="live_radio0">
									<span>
										<strong>
                            ...
                            <div class="text-center">
                                <button type="submit" @if (Helper::userWallet('balance') == 0) disabled
                                        @endif id="payLiveBtn" class="btn btn-primary mt-4 payLiveBtn">
                                    <i></i> {{\Stichoza\GoogleTranslate\GoogleTranslate::trans(trans('general.pay'), config("app.locale"))}} {{Helper::amountFormatDecimal($live->price, true)}}
                                    <small>{{$settings->currency_code}}</small>
                                </button>

                                <div class="w-100 mt-2">
                                    <a href="{{ url('/') }}"
                                       class="btn e-none p-0">{{\Stichoza\GoogleTranslate\GoogleTranslate::trans(trans('admin.cancel'), config("app.locale"))}}</a>
                                </div>
                            </div>
                        </form>
                    </div>
