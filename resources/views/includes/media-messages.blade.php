    ...
    @foreach ($mediaImageVideo as $media)
        ...
        @if ($media->type == 'image')
            <div class="media-grid-1">
                <a href="{{ $urlImg }}" class="media-wrapper glightbox {{$styleImgVertical}}"
                   data-gallery="gallery{{$msg->id}}" style="background-image: url('{{$urlImg}}?w=960&h=980')">
                    <img src="{{$urlImg}}?w=960&h=980"
                         {!! $media->width ? 'width="'. $media->width .'"' : null !!} {!! $media->height ? 'height="'. $media->height .'"' : null !!} class="post-img-grid"
                         alt="">
                </a>
            </div>
        @endif
        ...
        @if ($media->type == 'video')
            <div class="container-media-msg h-auto">
                <video class="js-player {{$classInvisible}}" controls style="height: 400px;"
                       @if ($media->video_poster) data-poster="{{ Helper::getFile(config('path.messages').$media->video_poster) }}" @endif>
    ...
    <div class="media-grid-{{ $mediaImageVideoTotal > 4 ? 4 : $mediaImageVideoTotal }}">
        ...
        @foreach ($mediaImageVideo as $media)
            ...
            @if ($media->type == 'image' || $media->type == 'video')
                ...
                <a href="{{ $urlMedia }}" class="media-wrapper glightbox" data-gallery="gallery{{$msg->id}}"
                   style="background-image: url('{{ $videoPoster ?? $urlMedia }}?w=960&h=980')">
                    ...
                    @if ($videoPoster)
                        <img src="{{ $videoPoster ?? $urlMedia }}?w=960&h=980"
                             {!! $media->width ? 'width="'. $media->width .'"' : null !!} {!! $media->height ? 'height="'. $media->height .'"' : null !!} class="post-img-grid"
                             alt="">
                    @endif
                </a>
...
@foreach ($msg->media as $media)
    ...
    @if ($media->type == 'zip')
        <a href="{{url('download/message/file', $msg->id)}}"
           class="d-block text-decoration-none @if ($mediaCount >= 2) mt-2 @endif">
            <div class="card">
                ...
                <div class="card-body py-2 px-4">
                    <h6 class="card-title text-primary text-truncate mb-0">
                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" fill="currentColor"
                             class="bi bi-coin mr-1" viewBox="0 0 16 16">
                            <path
