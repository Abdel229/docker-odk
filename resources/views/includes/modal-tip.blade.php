            ...
            <div class="modal-body p-0">
                <div class="card bg-white shadow border-0">
                    ...
                    <div class="card-body px-lg-5 py-lg-5 position-relative">
                        ...
                        <div class="text-muted text-center mb-3 position-relative modal-offset">
                            <img src="{{Helper::getFile(config('path.avatar').auth()->user()->avatar)}}" width="100"
                                 class="avatar-modal rounded-circle mb-1" alt="">
                            <h6>
                                {{\Stichoza\GoogleTranslate\GoogleTranslate::trans(trans('general.send_tip'), config("app.locale"))}}
                        ...
                        <form method="post" action="{{url('send/tip')}}" id="formSendTip">
                            ...
                            <input type="hidden" id="cardholder-name" value="{{ auth()->user()->name }}"/>
                            <input type="hidden" id="cardholder-email" value="{{ auth()->user()->email }}"/>
                            <input type="number" min="{{$settings->min_donation_amount}}"
                                   ...
                                   placeholder="{{\Stichoza\GoogleTranslate\GoogleTranslate::trans(trans('general.tip_amount'), config("app.locale"))}} ({{ \Stichoza\GoogleTranslate\GoogleTranslate::trans(__('general.minimum'), config("app.locale")) }} {{ Helper::amountWithoutFormat($settings->min_tip_amount) }})">
                            @csrf
                            ...
                            @if (! request()->route()->named('live'))
                                ...
                                @foreach (PaymentGateways::where('enabled', '1')->whereSubscription('yes')->get() as $payment)
                                    ...
                                    @php
                                        ...
                                        if ($payment->type == 'card' ) {
                                            $paymentName = '<i class="far fa-credit-card mr-1"></i> '.\Stichoza\GoogleTranslate\GoogleTranslate::trans(trans('general.debit_credit_card'), config("app.locale")) .' <small class="w-100 d-block">'.\Stichoza\GoogleTranslate\GoogleTranslate::trans(__('general.powered_by'), config("app.locale")).' '.$payment->name.'</small>';
                                        } else if ($payment->id == 1) {
                                            $paymentName = '<img src="'.url('img/payments', auth()->user()->dark_mode == 'off' ? $payment->logo : 'paypal-white.png').'" width="70"/> <small class="w-100 d-block">'.\Stichoza\GoogleTranslate\GoogleTranslate::trans(trans('general.redirected_to_paypal_website'), config("app.locale")).'</small>';
                                        } else {
                                    ...
                                    @endphp
                                    <div class="custom-control custom-radio mb-3">
                                        <input name="payment_gateway_tip" value="{{$payment->id}}"
                                               id="tip_radio{{$payment->id}}"
                                               @if ($allPayments->count() == 1 && Helper::userWallet('balance') == 0) checked
                            ...
                            @if ($settings->disable_wallet == 'on' && Helper::userWallet('balance') != 0 || $settings->disable_wallet == 'off')
                                <div class="custom-control custom-radio mb-3">
                                    <input name="payment_gateway_tip" @if (Helper::userWallet('balance') == 0) disabled
                                           @endif value="wallet" id="tip_radio0" class="custom-control-input"
                                           type="radio">
                                    <label class="custom-control-label" for="tip_radio0">
									<span>
										<strong>
