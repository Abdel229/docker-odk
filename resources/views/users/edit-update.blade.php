...
=======
    <section class="section section-sm">
        <div class="container">
            <div class="row justify-content-center text-center mb-sm">
                <div class="col-lg-12 py-5">
                    <h2 class="mb-0 font-montserrat">{{\Stichoza\GoogleTranslate\GoogleTranslate::trans(__('general.edit_post'), config('app.locale'))}}</h2>
                    <p class="lead text-muted mt-0"><a href="{{url()->previous()}}"><i
                                class="fas fa-arrow-left"></i> {{\Stichoza\GoogleTranslate\GoogleTranslate::trans(__('general.go_back'), config('app.locale'))}}
                        </a></p>
                </div>
            </div>
                ...
                <span class="rounded-circle mr-3">
            				<img src="{{ Helper::getFile(config('path.avatar').auth()->user()->avatar) }}"
                                 class="rounded-circle" width="60" height="60">
                                    ...
                                    <div class="media-body">
                                        <textarea name="description" id="updateDescription"
                                                  data-post-length="{{$settings->update_length}}" rows="5" cols="40"
                                                  placeholder="{{\Stichoza\GoogleTranslate\GoogleTranslate::trans(__('general.write_something'), config('app.locale'))}}"
                                                  class="form-control textareaAutoSize  border-0">{{$data->description}}</textarea>
                                    </div>
                                </div><!-- media -->

                                <input class="custom-control-input d-none" id="customCheckLocked" type="checkbox"
                                       {{$data->locked == 'yes' ? 'checked' : ''}}  name="locked"
                                       value="{{\Stichoza\GoogleTranslate\GoogleTranslate::trans('yes', config('app.locale'))}}">
                                ...
                                <div class="alert alert-success display-none"
                                     id="successUpdatePost">{{ \Stichoza\GoogleTranslate\GoogleTranslate::trans(__('admin.success_update'), config('app.locale')) }}
                                    <a href="{{ url($data->user()->username.'/post', $data->id) }}"
                                       class="link-border text-white">{{ \Stichoza\GoogleTranslate\GoogleTranslate::trans(__('general.go_to_post'), config('app.locale')) }}</a>
                                </div>
                                ...
                                <div class="justify-content-between align-items-center">
                                    ...
                                    <div class="form-group @if ($data->price == 0.00) display-none @endif" id="price">
                                        <div class="input-group mb-2">
                                            ...
                                            <input class="form-control isNumber"
                                                   value="{{$data->price != 0.00 ? $data->price : null}}"
                                    ...
                                    <div class="w-100">
                                        ...
                                        <a href="javascript:void(0)" id="removePhoto"
                                           ...
                                           data-placement="top"
                                           title="{{\Stichoza\GoogleTranslate\GoogleTranslate::trans(__('general.delete'), config('app.locale'))}}"><i
                                    ...
                                    @if (auth()->user()->subscriptionsActive() && $settings->users_can_edit_post == 'on'
                                        ...
                                        <button type="button"
                                                ...
                                                title="{{\Stichoza\GoogleTranslate\GoogleTranslate::trans($data->image == '' && $data->video == '' && $data->music == '' ? trans('general.upload_media') : trans('general.replace_media'), config('app.locale'))}}">
                                            <i class="feather icon-image f-size-25"></i>
                                        </button>
                                        ...
                                        <button type="button"
                                                ...
                                                data-toggle="tooltip" data-placement="top"
                                                title="{{\Stichoza\GoogleTranslate\GoogleTranslate::trans(__('general.upload_file_zip'), config('app.locale'))}}"
                                        ...
                                        @if ($data->price == 0.00)
                                            <button type="button" id="setPrice"
                                                    class="btn btn-upload btn-tooltip e-none align-bottom @if (auth()->user()->dark_mode == 'off') text-primary @else text-white @endif rounded-pill"
                                                    data-toggle="tooltip" data-placement="top"
                                                    title="{{\Stichoza\GoogleTranslate\GoogleTranslate::trans(__('general.set_price_for_post'), config('app.locale'))}}">
                                                <i class="feather icon-tag f-size-25"></i>
                                            </button>
                                        @endif
                                    ...
                                    <button type="button" id="contentLocked"
                                            ...
                                            data-toggle="tooltip" data-placement="top"
                                            title="{{\Stichoza\GoogleTranslate\GoogleTranslate::trans(__('users.locked_content'), config('app.locale'))}}">
                                        <i class="feather icon-{{$data->locked == 'yes' ? '' : 'un'}}lock f-size-25"></i>
                                    </button>

                                    <div class="d-inline-block float-right mt-3">
                                        <button type="submit" class="btn btn-sm btn-primary rounded-pill float-right"
                                                id="btnEditUpdate">
                                            <i></i> {{\Stichoza\GoogleTranslate\GoogleTranslate::trans(__('users.save'), config('app.locale'))}}
                                        </button>
